...
$optionImg=",`img_url`=$_POST[""]";
$details=$_POST["Details-input"];
$catId=(int)$_POST["category"];
$id=(int)$_POST["selected_product_to_update"];
UPDATE `products` SET `NAME`=$name,`price`=$price,`provider`=$provider,`description`=$descriptions $optionImg,`details`=$details,`category_id`=$catId WHERE `id`=$id;